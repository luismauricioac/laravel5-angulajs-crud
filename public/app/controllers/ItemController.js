...
app.controller('ItemController', function(dataFactory,$scope,$http){
	...
	function getResultsPage(pageNumber) {
		var apiPath = '/laravel5_angulajs_crud/public';
		if(! $.isEmptyObject($scope.libraryTemp)){
				dataFactory.httpRequest(apiPath+'/items?search='+$scope.searchText+'&page='+pageNumber).then(function(data) {
				$scope.data = data.data;
				$scope.totalItems = data.total;
			});
		}else{
			dataFactory.httpRequest(apiPath+'/items?page='+pageNumber).then(function(data) {
			$scope.data = data.data;
			$scope.totalItems = data.total;
			});
		}
